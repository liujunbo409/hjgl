...
@section('content')
    <a class="btn btn-success radius r btn-refresh" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" onclick="location.replace('{{URL::asset('admin/article/articleList')}}?id={{$data['id']}}');">
        <i class="Hui-iconfont">&#xe68f;</i></a>
    <br/><br/>
    <div class="text-c">
        <form action="{{URL::asset('admin/article/articleList')}}" method="get" class="form-horizontal">
            {{csrf_field()}}
...
<table class="table table-border table-bordered table-bg table-sort mt-10">
    <thead>
    <tr>
        <a href="javascript:;" style="margin-top: 5px" onclick="edit('添加文章','{{URL::asset('admin/article/addArticle')}}?id={{$data['id']}}')"
           class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;</i> 添加新文章
        </a>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="{{URL::asset('admin/article/sort')}}?id={{$data['id']}}" style="margin-top: 5px" class="btn btn-primary radius">
            <i class="Hui-iconfont">&#xe600;</i> 排序
        </a>
        <span class="r" style="margin-top: 20px">共有数据：<strong>{{$articles->total()}}</span>
        {{--<a href="javascript:;" onclick="edit('添加文章','{{URL::asset('admin/article/chooseArticle')}}?id={{$data['id']}}')"--}}
           {{--class="btn btn-primary radius">--}}
