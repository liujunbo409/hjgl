...
@section('content')
    ...
    <div class="page-container">
        <div class="text-c">
            <form action="{{URL::asset('admin/tool/chooseShop')}}" method="post" class="form-horizontal">
                {{csrf_field()}}
                ...
                <tr class="text-c">
                    ...
                    <td class="td-manage">
                        @if(!empty($tool->shop_id))
                            已选择商家
                        @else
                            <span style="color:blue;cursor: pointer;" onclick="chooseCon('{{$data->id}}','{{$tool->id}}')">选择</span>
                        @endif
                    </td>
                </tr>
            @endforeach
