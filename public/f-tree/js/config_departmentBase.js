...
var changeClass = function (obj){
   $(".f-treeList-title").removeClass('choicNode');
    $(obj).parent().addClass('choicNode');
}
configFun.prototype = {
    data:{
        tree:[],
        treeStorage:''//瀛樺偍鐨勭埗绾ф爲鑺傜偣
    },
    show:function(reg){
        var t = this;
        if(t.isArray(reg)){
    ...
    tree:function(res){
        ...
        for(var i=0; i<data.length; i++){
            if(data[i].pid == 0){
                ...
                var listHtml = '';
                if(res.treeType=='mulu'){//树类型--目录附加目录下文章数
                    if(res.type == 0){
                        listHtml = $('<div class="f-treeList f-treeListOne">'+
                            ...
                            ' <p class="f-treeList-titleP" onClick="clickTree('+data[i].id+','+data[i].pid+');changeClass(this)"  style="cursor: pointer;" href="javascript:;" title="'+data[i].title+'">'+ data[i].title+'('+data[i].num+')' +'</p>'
                            + '</div></div></div>');
                    }
                    else{
                        var radioIconHtml = '';
                        if(res.type == 1){
                            radioIconHtml = '<span class="f-iconRadio"></span>';
                        }
                        else{
                            radioIconHtml = '<span class="f-iconRadio"></span>';
                        }
                ...
                else if(res.treeType=='illness'){//树类型--疾病树附加是否为基础疾病图标
                    if(res.type == 0){
                        if(data[i].isBase=='1'){//是基础疾病
                            listHtml = $('<div class="f-treeList f-treeListOne">'+
                                '<div class="f-treeList-top">'+
                                '<div class="f-treeList-old"><span class="f-iconJia"></span></div>'+
                                '<div class="f-treeList-title">'+
                                '<div class="f-treeList-titleImg" ><span class="f-iconBi"></span></div>'+
                                ' <p class="f-treeList-titleP" onClick="clickTree('+data[i].id+','+data[i].pid+');changeClass(this)"  style="cursor: pointer;" href="javascript:;" title="'+data[i].title+'">'+ data[i].title +'</p>'
                                + '<span class="f-iconBi" style="float: right;margin-top: 2px;"></span>'
                                + '</div></div></div>');
                        }
                        else{
                            listHtml = $('<div class="f-treeList f-treeListOne">'+
    ...
    treeFor:function(res){
        ...
        if(self.length == 0 && res.dataHover.pid != 0){
            ...
            var selfWrapper = selfDom.parent('.f-treeListWrapper');//搴曠骇鎵€鍦ㄧ殑瀹瑰櫒
